...
foreach ($dir in $dirs) {
    if (Test-Path $dir) {
        ...
        Write-Host "✗ $dir MISSING - Creating..."
...
foreach ($pidFile in $pids) {
    if (Test-Path $pidFile) {
        ...
        if ($proc) {
            Write-Host "✓ $(Split-Path $pidFile -Leaf): PID $pidValue is RUNNING"
            $anyStarted = $true
        } else {
            Write-Host "✗ $(Split-Path $pidFile -Leaf): PID file exists ($pidValue) but process not running"
        }
    } else {
        Write-Host "✗ $(Split-Path $pidFile -Leaf): No PID file"
    }
}
...
foreach ($port in $ports) {
    try {
        ...
        Write-Host "✓ Port $port: Responding (status $($resp.StatusCode))"
    } catch {
        Write-Host "✗ Port $port: No response ($($_.Exception.GetBaseException().Message))"
    }
}
