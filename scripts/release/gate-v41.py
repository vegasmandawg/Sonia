"""
SONIA v4.1.0 Promotion Gate
============================
Schema v8.0 -- three gate classes (A/B/C), retry classifier, per-gate telemetry.

Inherits the full v4.0 floor (37 gates, 622 unit tests) and adds v4.1 delta gates.

FAIL-FAST: Any Class-A gate failure immediately marks HOLD.

Gate Classes:
  A -- Inherited baseline (37 from v4.0.0): fail-fast, always_retry
  B -- v4.1 delta gates (epic-owned): per-epic additions
  C -- Cross-cutting gates (evidence integrity): structural checks

Usage:
    python gate-v41.py [--output-dir DIR]
    python gate-v41.py --floor-only    # Class A only
    python gate-v41.py --delta-only    # Class B + C only
...
# ---- Class A: Inherited baseline gates (37 from v4.0.0) ---------------------
# 24 original (v3.7) + 4 delta (v3.8) + 4 delta (v3.9) + 3 delta (v4.0) + 1 evidence (v4.0) + 1 test floor
...
INHERITED_UNIT_TEST_FLOOR = 622

# ---- Class B: v4.1 delta gates (epic-owned) ---------------------------------

CLASS_B_GATES = [
    ("v41-epic1-provenance-gate.py", "gates", "E1: Governance Provenance Deepening"),
    ("v41-epic2-chaos-recovery-gate.py", "gates", "E2: Fault/Recovery Determinism Under Stress"),
    ("v41-epic3-repro-gate.py", "gates", "E3: Reproducible Release + Cleanroom Parity"),
]
...
CLASS_C_GATES = [
    ("v41-evidence-integrity-gate.py", "gates", "Evidence Integrity"),
]
...
def main():
    parser = argparse.ArgumentParser(description="SONIA v4.1 Promotion Gate")
    parser.add_argument(
        "--output-dir",
        default=str(REPO_ROOT / "reports" / "audit" / "v4.1-baseline"),
    )
    ...
    print("=" * 60)
    print("  SONIA v4.1 Promotion Gate (schema v8.0)")
    print("=" * 60)
    ...
    # ---- [3/4] Class B: v4.1 delta gates ----
    delta_pass = delta_fail = 0
    if not args.floor_only and CLASS_B_GATES:
        print(f"\n[3/4] Running {CLASS_B_COUNT} Class B (v4.1 delta) gates...")
        if class_a_hold:
    ...
    report_path = output_dir / f"gate-matrix-v41-{ts}.json"
